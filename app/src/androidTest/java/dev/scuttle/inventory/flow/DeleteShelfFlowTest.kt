...
class DeleteShelfFlowTest : FlowTestBase() {
    ...
    fun select_and_delete_shelf_removes_it_from_tabs() {
        ...
        composeRule.apply {
            ...
            // Tap "Select shelves to delete" icon → delete mode
            onNodeWithContentDescription("Select shelves to delete").performClick()
            waitForIdle()

            // Select "Top shelf" tab → becomes selected
            onNodeWithText("Top shelf").performClick()
            waitForIdle()

            // DELETE /households/1/locations/10/shelves/100 → 204; shelf removed optimistically
